...
fill(axA, [xAxis fliplr(xAxis)], [S.Death.lo0 fliplr(S.Death.hi0)], ...
    [0.21 0.49 0.72], 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'HandleVisibility', 'off');
fill(axA, [xAxis fliplr(xAxis)], [S.Death.lo1 fliplr(S.Death.hi1)], ...
    [0.89 0.29 0.20], 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'HandleVisibility', 'off');

plot(axA, xAxis, S.Death.m0, '-o',  'Color', '[0.21 0.49 0.72]', 'LineWidth', 1.5);
plot(axA, xAxis, S.Death.m1, '-s',  'Color', '[0.89 0.29 0.20]', 'LineWidth', 1.5);

xlabel(axA, 'Trial index', 'FontSize', labelFS);
ylabel(axA, 'RT-scale', 'FontSize', labelFS);
set(axA, 'FontSize', tickFS);
title(axA, 'Death + Me', 'FontSize', labelFS, 'FontWeight', 'normal');


xlim(axA, [1 20]);
xticks(axA, [1 5 10 15 20]);
ylim(axA, [0.90 1.40]);
ylA = ylim(axA);
yticks(axA, linspace(ylA(1), ylA(2), 5));
ytickformat(axA, '%.2f');

patch(axA, [3.5 6 6 3.5], [ylA(1) ylA(1) ylA(2) ylA(2)], ...
    [1.00 0.88 0.60], 'FaceAlpha', 0.35, 'EdgeColor', 'none', ...
    'HandleVisibility', 'off');

set(figA, 'PaperPositionMode', 'auto');
set(figA, 'Units', 'pixels');
pos = get(figA, 'Position');
set(figA, 'Position', [pos(1) pos(2) pos(3) pos(3)/1.786]);
...
plot(axB, xAxis, S.Life.m0, '-o',  'Color', [0.21 0.49 0.72], 'LineWidth', 1.5);
plot(axB, xAxis, S.Life.m1, '-s',  'Color', [0.89 0.29 0.20], 'LineWidth', 1.5);

xlabel(axB, 'Trial index', 'FontSize', labelFS);
ylabel(axB, 'RT-scale', 'FontSize', labelFS);
set(axB, 'FontSize', tickFS);
title(axB, 'Life + Me', 'FontSize', labelFS, 'FontWeight', 'normal');

xlim(axB, [1 20]);
xticks(axB, [1 5 10 15 20]);

ylim(axB, [0.90 1.40]);
ylB = ylim(axB);
yticks(axB, linspace(ylB(1), ylB(2), 5));
ytickformat(axB, '%.2f');
...
set(figB, 'Units', 'pixels');
pos = get(figB, 'Position');
set(figB, 'Position', [pos(1) pos(2) pos(3) pos(3)/1.786]);
...
plot(axC, xAxis, S.Death.e0, '-o', 'Color', [0.21 0.49 0.72], 'LineWidth', 1.5);
plot(axC, xAxis, S.Death.e1, '-s', 'Color', [0.89 0.29 0.20], 'LineWidth', 1.5);
...
xlim(axC, [1 20]);
xticks(axC, [1 5 10 15 20]);
ylim(axC, [-0.26 -0.16]);
ylC = ylim(axC);
yticks(axC, linspace(ylC(1), ylC(2), 5));
ytickformat(axC, '%.2f');
...
set(figC, 'Units', 'pixels');
pos = get(figC, 'Position');
set(figC, 'Position', [pos(1) pos(2) pos(3) pos(3)/1.786]);
...
plot(axD, xAxis, S.Life.e0, '-o', 'Color', [0.21 0.49 0.72], 'LineWidth', 1.5);
plot(axD, xAxis, S.Life.e1, '-s', 'Color', [0.89 0.29 0.20], 'LineWidth', 1.5);
...
xlim(axD, [1 20]);
xticks(axD, [1 5 10 15 20]);
ylim(axD, [-0.26 -0.16]);
ylD = ylim(axD);
yticks(axD, linspace(ylD(1), ylD(2), 5));
ytickformat(axD, '%.2f');
...
set(figD, 'Units', 'pixels');
pos = get(figD, 'Position');
set(figD, 'Position', [pos(1) pos(2) pos(3) pos(3)/1.786]);
