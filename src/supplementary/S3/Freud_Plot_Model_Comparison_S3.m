%% Freud_Plot_Model_Comparison_S3.m
% Generate the diagnostic-group ROC panel for Figure S3C.
%
...
%
% For the supplementary Figure S3 analysis, the same feature construction
% and classifier pipeline are evaluated under the S3 grouping convention.
% This mirrors the relationship between Freud_PCA_Trial_Dynamics.m and
% Freud_PCA_Trial_Dynamics_S3.m: the computational pipeline is retained,
...
fprintf('\n===== FIGURE S3C DATA SUMMARY =====\n');
fprintf('N=%d | active_score classes=[%g,%g] -> positive class is %g\n', N, cA, cB, cB);
fprintf('S3 grouping convention: y=0 = Control, y=1 = MDD\n');
...
%% ----------------------- Save J=2 ALT-learned-v package for Figure S3C ---
saveFile = 'Freud_Model_J2_Latents_S3.mat';
Jsave = 2;

ALT_J2_S3 = struct();
ALT_J2_S3.cfg     = cfg_alt_J2_used;       % exact cfg used for J=2
ALT_J2_S3.roc_fpr = ROC.alt(Jsave).fpr;
ALT_J2_S3.roc_tpr = ROC.alt(Jsave).tpr;
ALT_J2_S3.auc     = ROC.alt(Jsave).auc;
ALT_J2_S3.thrStar = ROC.alt(Jsave).thr;
ALT_J2_S3.sens    = ROC.alt(Jsave).sens;
ALT_J2_S3.spec    = ROC.alt(Jsave).spec;
ALT_J2_S3.balacc  = ROC.alt(Jsave).balacc;
ALT_J2_S3.scores  = ROC.alt(Jsave).scores; % p_all
ALT_J2_S3.labels  = ROC.alt(Jsave).labels; % y
ALT_J2_S3.results = ROC.alt(Jsave).results;

save(saveFile, 'ALT_J2_S3');
fprintf('\nSaved J=2 PC learned package for Figure S3C to:\n  %s\n', saveFile);

%% ----------------------- Plot Figure S3C (ONE panel, 6 curves) ----------
...
% ==========================================================
% Highlight ALT J=2  (the primary S3 comparison model)
% ==========================================================
...
save('Freud_ROC_Comparison_Data_S3.mat', 'ROC', 'cfg_alt', 'cfg_fixed', 'alpha_alt', 'alpha_fixed');
fprintf('Saved all ROC curves/data to: Freud_ROC_Comparison_Data_S3.mat\n');
