...
%   outPrefix     : output prefix, e.g., 'Figure_2_'
    ...
    % --- Lowered font sizes specifically for Figure 2D ---
    labelFS_D  = 24;
    tickFS_D   = 20;
    legendFS_D = 20;

    % --- Original font sizes for Figure 2E ---
    labelFS  = 36;
    tickFS   = 30;
    legendFS = 26;
    ...
    xlim(ax, [1 8]);
    ylim(ax, [-0.4 0.3]);
    xlabel(ax, 'Lag (blocks)', 'FontSize', labelFS_D);
    ylabel(ax, 'Autocorrelation', 'FontSize', labelFS_D);
    set(ax, 'FontSize', tickFS_D, 'LineWidth', 1);
    yticks(ax, [-0.5 -0.25 0 0.25 0.5]);
    lg = legend(ax, {'SI-', 'SI+'});
    set(lg, 'FontSize', legendFS_D, 'Box', 'off');
    set(lg, 'Position', [0.75, 0.85, 0.2, 0.1]);
    set(gcf, 'PaperPositionMode', 'auto');
    set(gcf, 'Units', 'pixels');
    pos = get(gcf, 'Position');
    set(gcf, 'Position', [pos(1) pos(2) pos(3) pos(3)/2.08]);
    ...
    grid(ax2, 'off');
    box(ax2, 'off');
    hHist = histogram(ax2, nullVals, 40, 'Normalization', 'pdf', 'FaceColor', [0.6 0.6 0.6], 'EdgeColor', [0.6 0.6 0.6]);
    yl = ylim(ax2);
    hObs = plot(ax2, [obsVal obsVal], yl, 'k-', 'LineWidth', 5);
    plot(ax2, [muNull muNull], yl, 'k--', 'LineWidth', 2);
    xlabel(ax2, 'Rhythm index', ...
        'FontSize', labelFS);
    ...
    xl = xlim(ax2);
    xlim(ax2, [-0.3 xl(2)]);
    legStrPerm = sprintf('Permuted (\\mu = %.3f, \\sigma = %.3f)', muNull, sdNull);
