...
lg = legend(ax, {'Lower SI', 'Higher SI'}, 'Box', 'off');
set(lg, 'FontSize', legendFS);

set(lg, 'Units', 'normalized', 'Position', [0.70, 0.75, 0.20, 0.10]);
...
hHistB = histogram(axB, nullValsB, 40, 'Normalization', 'pdf', 'FaceColor', [0.6 0.6 0.6], 'EdgeColor', [0.6 0.6 0.6]);

ylB = ylim(axB);
hObsB = plot(axB, [obsValB obsValB], ylB, 'k-', 'LineWidth', 5);
plot(axB, [muNullB muNullB], ylB, 'k--', 'LineWidth', 2);

xlabel(axB, 'Rhythm index', 'FontSize', labelFS);
...
legStrB = sprintf('Permuted (\\mu = %.3f, \\sigma = %.3f)', muNullB, sdNullB);
...
set(fB, 'Units', 'pixels');
pos = get(fB, 'Position');
set(fB, 'Position', [pos(1) pos(2) pos(3) pos(3)/2.08]);
