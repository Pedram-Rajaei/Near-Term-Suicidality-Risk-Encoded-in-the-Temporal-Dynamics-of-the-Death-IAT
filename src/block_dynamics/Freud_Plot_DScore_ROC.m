...
% Generate Figure 2A and Figure 2B from processed BDIAT data.
%
% This script computes a subject-level D-score from Death + Me and Life + Me
% reaction-time trials, then exports:
%   Figure_2_A.svg : D-score group boxplot
%   Figure_2_B.svg : D-score ROC curve
%
% Required input:
...
axA = axes('Parent', figA); hold(axA,'on'); grid(axA,'off'); box(axA,'off');
...
plot(axA, x1, D_inact, '.', 'Color', [0.21 0.49 0.72], 'MarkerSize', 18);
plot(axA, x2, D_act,   '.', 'Color', [0.89 0.29 0.20], 'MarkerSize', 18);
...
axB = axes('Parent', figB); hold(axB,'on'); grid(axB,'off'); box(axB,'off');
...
% ROC curve.
hROC = plot(axB, FPR, TPR, 'LineWidth', 4, 'Color', 'k');
