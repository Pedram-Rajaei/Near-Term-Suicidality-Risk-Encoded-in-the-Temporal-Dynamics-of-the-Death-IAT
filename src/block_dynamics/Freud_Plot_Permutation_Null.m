...
starColor = [1 1 1];       % magenta observed marker
...
% Plot the histogram
histogram(ax1, null_auc, 40, 'FaceColor', [0.6 0.6 0.6], 'EdgeColor', [0.6 0.6 0.6]);

% REPLACE xline WITH plot
% Use the current ylim to ensure the line spans the entire y-range
yLimits = ylim(ax1);
plot(ax1, [auc_obs auc_obs], yLimits, 'k-', 'LineWidth', 5);
...
% Note: If you set xlim/ylim AFTER plotting, ensure the line 
% coordinates are within the final axis range.
xlim(ax1, [0.25 0.82]);
ylim(ax1, [0 70]);
...
% Plot the histogram
histogram(ax2, null_bacc, 40, 'FaceColor', [0.6 0.6 0.6], 'EdgeColor', [0.6 0.6 0.6]);

% Replace xline with plot using the explicit axis limits
% Using [0 100] as specified in your ylim call
plot(ax2, [bacc_obs bacc_obs], [0 100], 'k-', 'LineWidth', 5);
