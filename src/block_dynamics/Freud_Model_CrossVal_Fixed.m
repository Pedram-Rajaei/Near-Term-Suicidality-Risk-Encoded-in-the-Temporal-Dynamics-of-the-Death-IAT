...
%FREUD_MODEL_CROSSVAL_FIXED Leave-one-out cross-validation for fixed-PC classifier.
%
% This function evaluates a fixed-projection joint classifier using LOOCV.
% For each held-out subject, block-space projection vectors are estimated
% from the training subjects only, then sparse logistic regression is fit on
...
%
% Output:
%   results : structure with LOOCV probabilities and predicted labels.

    if isfield(cfg,'rng_seed') && ~isempty(cfg.rng_seed)
        rng(cfg.rng_seed,'twister');   % Reproducible fold-level training.
    end
...
%   C = (1/Ntr) * sum_i (Xi * Xi')
    ...
    % v2..vJ are orthogonalized against previously selected directions.
    for j = 2:J
        vj = U(:,j);
        for k = 1:(j-1)
...
% Thin wrapper around lassoglm for L1-penalized logistic regression.
    ...
    lambdaRatio = 1e-3;   % Lower bound for the regularization path.
    ...
    [B,FitInfo] = lassoglm( ...
        ...
        'CV', cvFolds, ...
        'Weights', w_tr, ...
        'Standardize', false, ...          % Features are standardized before this call when requested.
        'NumLambda', numLambda, ...
