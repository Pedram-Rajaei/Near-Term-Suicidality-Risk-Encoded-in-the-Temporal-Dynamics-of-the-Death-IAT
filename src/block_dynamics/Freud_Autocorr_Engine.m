function out = Freud_Autocorr_Engine(X, G, varargin)
%FREUD_AUTOCORR_ENGINE Compute trial- and block-level ACF summaries.
%
% This legacy helper computes autocorrelation functions (ACFs) from the
% reaction-time matrix at two resolutions:
%   1. Trial-level ACF across the full trial sequence.
%   2. Block-level ACF from per-block subject summaries.
%
% Block-level summaries are computed using the mean or median within each
% block, optionally log-transformed, detrended, and mean-centered before ACF
% estimation. The function can optionally generate group-mean diagnostic
% ACF plots with SEM bands.
%
% Inputs:
%   X : subjects x trials reaction-time matrix
%   G : subjects x 1 group vector, where 0 = SI- and 1 = SI+
%
% Output:
%   out : structure containing ACF arrays, lag vectors, block summaries,
%         detrended block summaries, and group-level mean/SEM statistics.
%
% Note:
%   This is a lightweight diagnostic helper. The main block-level rhythm,
%   permutation, and shuffle analyses are implemented in
...
if makePlots
    if doBlock
        ...
        hold on;
        plot(lagsBlock, out.groupMeanBlockACF0, '-o', 'DisplayName','Group 0');
        plot(lagsBlock, out.groupMeanBlockACF1, '-o', 'DisplayName','Group 1');
        ...
        if plotSEM
            ...
            fill([x; flipud(x)], [m0-s0; flipud(m0+s0)], 'k', ...
                'FaceAlpha', 0.10, 'EdgeColor','none', 'HandleVisibility','off');
            fill([x; flipud(x)], [m1-s1; flipud(m1+s1)], 'k', ...
                'FaceAlpha', 0.10, 'EdgeColor','none', 'HandleVisibility','off');
        end
    ...
    if doTrial
        ...
        hold on;
        plot(lagsTrial, out.groupMeanTrialACF0, 'DisplayName','Group 0');
        plot(lagsTrial, out.groupMeanTrialACF1, 'DisplayName','Group 1');
