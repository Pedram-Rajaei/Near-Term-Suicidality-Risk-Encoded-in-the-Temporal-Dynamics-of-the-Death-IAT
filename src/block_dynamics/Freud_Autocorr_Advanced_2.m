...
%FREUD_AUTOCORR_ADVANCED_2 Compute trial- and block-level autocorrelation summaries.
%
...
% estimation. The function can also generate group-mean ACF diagnostic plots
% with SEM bands.
%
% Inputs:
...
%         and group-level mean/SEM summaries.
%
% Note:
%   This is a legacy/diagnostic helper. The main block-level rhythm and
%   shuffle analyses are implemented in Freud_Autocorr_Advanced.m.
...
if makePlots
    if doBlock
        ...
        if plotSEM
            ...
            % Draw ribbons first
            fill([x; flipud(x)], [m0-s0; flipud(m0+s0)], c0, ...
                'FaceAlpha', 0.18, 'EdgeColor','none', 'HandleVisibility','off');
            fill([x; flipud(x)], [m1-s1; flipud(m1+s1)], c1, ...
                'FaceAlpha', 0.18, 'EdgeColor','none', 'HandleVisibility','off');
        end
    
        % Draw lines after ribbons
        plot(lagsBlock, out.groupMeanBlockACF0, '-o', ...
            'Color', c0, 'LineWidth', 1.5, 'DisplayName','Inactive SI');
        plot(lagsBlock, out.groupMeanBlockACF1, '-o', ...
            'Color', c1, 'LineWidth', 1.5, 'DisplayName','Active SI');
