%% Freud_Trial_Dynamics_TTest_S8.m
% Trial-wise statistical tests for Figure S8.
%
...
% Figure formatting for exported S8 panels.
labelFS  = 24;
tickFS   = 20;
legendFS = 20;
...
for sIdx = 1:numel(startSets)
    ...
    % Store FDR-adjusted q-values for Figure S8 export.
...
%% Export Figure S8 panels
assert(isfield(S,'Death_ttest') && isfield(S,'Life_ttest'), ...
    'Need S.Death_ttest and S.Life_ttest stored before exporting Figure_S8 SVGs.');
...
% helper: one exporter
exportS8panel('Figure_S8_A.svg', xAxis, S.Death_ttest.q_mean, yl_alpha, W, H, ...
    'Trial index', 'q-value', {'qFDR(rowMean)'}, labelFS, tickFS, legendFS);

exportS8panel('Figure_S8_B.svg', xAxis, S.Life_ttest.q_mean, yl_alpha, W, H, ...
    'Trial index', 'q-value', {'qFDR(rowMean)'}, labelFS, tickFS, legendFS);

exportS8panel('Figure_S8_C.svg', xAxis, S.Death_ttest.q_ev, yl_alpha, W, H, ...
    'Trial index', 'q-value', {'qFDR(rowEV)'}, labelFS, tickFS, legendFS);

exportS8panel('Figure_S8_D.svg', xAxis, S.Life_ttest.q_ev, yl_alpha, W, H, ...
    'Trial index', 'q-value', {'qFDR(rowEV)'}, labelFS, tickFS, legendFS);

disp('Saved t-test SVGs: Figure_S8_A.svg, Figure_S8_B.svg, Figure_S8_C.svg, Figure_S8_D.svg');

%% ---------------- local helper functions ----------------
function exportS8panel(outSvg, xAxis, qVals, yl_alpha, W, H, xlab, ylab, legTxt, labelFS, tickFS, legendFS)
