...
%
% Requirements:
%   - Freud_Processed_BDIAT.mat contains: XF, active_score
%   - Freud_Model_CrossVal_Joint.m, Freud_Model_CrossVal_Fixed.m are on MATLAB path
...
% Plot all ALT curves (J=1..3)
for J = 1:3
    plot(ax, ROC.alt(J).fpr, ROC.alt(J).tpr, 'LineWidth', 2.2, ...
        'DisplayName', sprintf('PC learned, J=%d (AUC=%.3f)', J, ROC.alt(J).auc));
end

% Plot all FIXED curves (J=1..3)
for J = 1:3
    plot(ax, ROC.fix(J).fpr, ROC.fix(J).tpr, 'LineWidth', 2.2, ...
