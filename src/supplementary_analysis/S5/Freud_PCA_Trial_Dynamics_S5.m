...
% Diagnostic-group trial-level reaction-time dynamics for Figure S5.
%
...
%
%   Figure_S5_A.svg : Death + Me RT-scale trajectories by diagnostic group
%   Figure_S5_B.svg : FDR-adjusted q-values for Death + Me rowMean tests
%
% Required input:
...
for sIdx = 1:numel(startSets)
    ...
    % Store trial-wise p-values and FDR-adjusted q-values for S5B.
...
fill(axA, [xAxis fliplr(xAxis)], [S.Death.lo0 fliplr(S.Death.hi0)], [0.13 0.55 0.13], ...
    'FaceAlpha',0.15, 'EdgeColor','none', 'HandleVisibility','off');
fill(axA, [xAxis fliplr(xAxis)], [S.Death.lo1 fliplr(S.Death.hi1)], [0.49 0.18 0.56], ...
    'FaceAlpha',0.15, 'EdgeColor','none', 'HandleVisibility','off');
plot(axA, xAxis, S.Death.m0, '-o', 'Color', [0.13 0.55 0.13], 'LineWidth', 1.5);
plot(axA, xAxis, S.Death.m1, '-s', 'Color', [0.49 0.18 0.56], 'LineWidth', 1.5);
...
print(fA, 'Figure_S5_A.svg', '-dsvg');
close(fA);

disp('Saved SVG: Figure_S5_A.svg');

%% ======================= EXPORT: Figure S5_B as SVG (clean q-only style) =======================
assert(isfield(S,'Death_ttest') && isfield(S,'Life_ttest'), ...
    'Need S.Death_ttest and S.Life_ttest stored before exporting Figure_S5_B.svg.');
...
% Export only the Death + Me rowMean q-value panel for Figure S5B.
exportS5panel('Figure_S5_B.svg', xAxis, S.Death_ttest.q_mean, yl_alpha, W, H, ...
    'Trial index', 'q-value', {'qFDR(rowMean)'}, labelFS, tickFS, legendFS);

disp('Saved SVG: Figure_S5_B.svg');
