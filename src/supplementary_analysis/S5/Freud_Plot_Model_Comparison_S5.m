%% Freud_Plot_Model_Comparison_S5.m
% Generate the diagnostic-group ROC panel for Figure S5C.
%
% This script uses the same LOOCV classifier comparison pipeline as the main
% Figure 4B analysis:
%   1. Learned block-space projection, J = 1, 2, 3
%   2. Fixed-PC block-space projection, J = 1, 2, 3
%
% For the supplementary Figure S5 analysis, the same feature construction
% and classifier pipeline are evaluated under the S5 grouping convention.
% This mirrors the relationship between Freud_PCA_Trial_Dynamics.m and
% Freud_PCA_Trial_Dynamics_S5.m: the computational pipeline is retained,
% while the exported figure and group interpretation are changed.
%
% Required input:
...
fprintf('\n===== FIGURE S5C DATA SUMMARY =====\n');
fprintf('N=%d | active_score classes=[%g,%g] -> positive class is %g\n', N, cA, cB, cB);
fprintf('S5 grouping convention: y=0 = Control, y=1 = MDD\n');
fprintf('Counts: Control: %d | MDD: %d\n\n', sum(y==0), sum(y==1));
...
%% ----------------------- Save J=2 ALT-learned-v package for Figure S5C ---
saveFile = 'Freud_Model_J2_Latents_S5.mat';
Jsave = 2;

ALT_J2_S5 = struct();
ALT_J2_S5.cfg     = cfg_alt_J2_used;       % exact cfg used for J=2
ALT_J2_S5.roc_fpr = ROC.alt(Jsave).fpr;
ALT_J2_S5.roc_tpr = ROC.alt(Jsave).tpr;
ALT_J2_S5.auc     = ROC.alt(Jsave).auc;
ALT_J2_S5.thrStar = ROC.alt(Jsave).thr;
ALT_J2_S5.sens    = ROC.alt(Jsave).sens;
ALT_J2_S5.spec    = ROC.alt(Jsave).spec;
ALT_J2_S5.balacc  = ROC.alt(Jsave).balacc;
ALT_J2_S5.scores  = ROC.alt(Jsave).scores; % p_all
ALT_J2_S5.labels  = ROC.alt(Jsave).labels; % y
ALT_J2_S5.results = ROC.alt(Jsave).results;

save(saveFile, 'ALT_J2_S5');
fprintf('\nSaved J=2 PC learned package for Figure S5C to:\n  %s\n', saveFile);

%% ----------------------- Plot Figure S5C (ONE panel, 6 curves) ----------
...
for J = 1:3
    hAlt(J) = plot(ax, ROC.alt(J).fpr, ROC.alt(J).tpr, ...
        'LineWidth', 2.2, ...
        'DisplayName', sprintf('Tuned, J=%d (AUC=%.3f)', J, ROC.alt(J).auc));
end
...
for J = 1:3
    hFix(J) = plot(ax, ROC.fix(J).fpr, ROC.fix(J).tpr, ...
        'LineWidth', 2.2, ...
        'DisplayName', sprintf('Fixed, J=%d (AUC=%.3f)', J, ROC.fix(J).auc));
end

% ==========================================================
% Highlight ALT J=2  (the primary S5 comparison model)
% ==========================================================
...
save('Freud_ROC_Comparison_Data_S5.mat', 'ROC', 'cfg_alt', 'cfg_fixed', 'alpha_alt', 'alpha_fixed');
fprintf('Saved all ROC curves/data to: Freud_ROC_Comparison_Data_S5.mat\n');
